...
GLOBAL FLAGS:
  ...
  --turn <id>       Browser automation turn id (default SUPERDUCK_TURN_ID,
                    then session id when available).
...
func clientOpts() cliclient.Options {
	...
	return cliclient.Options{
		...
		SessionID:  sessionID,
		TurnID:     resolvedBrowserTurnID(sessionID),
	}
}
...
func resolvedBrowserTurnID(sessionID string) string {
	if gflags.TurnID != "" {
		return gflags.TurnID
	}
	if v := os.Getenv("SUPERDUCK_TURN_ID"); v != "" {
		return v
	}
	return sessionID
}
...
func newRandomCLISessionID() (string, error) {
	...
	return "cli:file:" + hex.EncodeToString(raw[:]), nil
}
