...
// eslint-disable-next-line no-misleading-character-class -- intentional invisible-char strip set
...
async function findCursorInteractiveElements(
  ...
  try {
    const scanFunc = (maxNodes: number) => {
      ...
      const nodeList = document.body ? document.body.querySelectorAll('*') : [];
      let elements: HTMLElement[] = Array.prototype.slice.call(nodeList) as HTMLElement[];
      // 大页面：仅扫描视口内元素（小红书等 SPA 常超阈值，但筛选浮层在视口内）
      if (elements.length > maxNodes) {
        const vh = window.innerHeight;
        const vw = window.innerWidth;
        const inView: HTMLElement[] = [];
        for (let vi = 0; vi < elements.length && inView.length < maxNodes; vi++) {
          const candidate = elements[vi];
          const crect = candidate.getBoundingClientRect();
          if (crect.width === 0 || crect.height === 0) continue;
          if (crect.bottom < 0 || crect.top > vh || crect.right < 0 || crect.left > vw) continue;
          inView.push(candidate);
        }
        elements = inView;
        if (elements.length === 0) return { count: 0, skipped: true };
      }
      let count = 0;
...
async function fetchAXTree(tabId: number): Promise<AXNode[]> {
  ...
  const childResults = await Promise.all(
    iframeNodes.map(async (ifNode, idx) => {
      ...
      let childNodes: AXNode[];
      try {
        const childResp = await cdpDebugger.sendCommand<CdpAccessibilityTreeResult<AXNode>>(
          tabId,
          'Accessibility.getFullAXTree',
          {
            frameId
