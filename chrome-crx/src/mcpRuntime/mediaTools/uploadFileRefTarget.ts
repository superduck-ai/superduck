...
/**
 * Resolve a file-input and its click target from a ref element. SINGLE source
 * of truth — the page-injected helpers below receive this as a serialized
 * string and rebuild it via `Function`, so any change here flows into the page
 * context without a hand-maintained copy.
 */
...
/** Serialized form injected into the page context (module closures don't survive serialization). */
export const resolveUploadFileRefTargetSource = resolveUploadFileRefTarget.toString();

/** Type alias kept inline-only so the file has no dangling forward references. */
type ResolveFn = typeof resolveUploadFileRefTarget;

/**
 * Injected via chrome.scripting.executeScript — must stay self-contained.
 * `resolveSource` is the serialized `resolveUploadFileRefTarget`, rebuilt in-page.
 */
export function markUploadFileRefInPage(
  ...
  const resolveUploadFileRefTarget = (
    new Function(`return (${resolveSource})`) as () => ResolveFn
  )();
...
export function markUploadFileAtCoordinateInPage(
  ...
  function scrollViewportPointIntoView(viewportX: number, viewportY: number): [number, number] {
    ...
    return [docX - window.scrollX, docY - window.scrollY];
  }

  function elementAtViewportPoint(viewportX: number, viewportY: number): Element | null {
    for (const el of document.elementsFromPoint(viewportX, viewportY)) {
  ...
  const [pointX, pointY] = scrollViewportPointIntoView(x, y);
  const hit = elementAtViewportPoint(pointX, pointY);
  if (!hit) {
  ...
  while (current) {
    const result = resolveUploadFileRefTarget(current, pathCount);
    if (!('error' in result)) {
      result.fileInput.setAttribute(uploadAttr, '1');
      const separateClickTarget = result.clickTarget !== result.fileInput;
      if (separateClickTarget) result.clickTarget.setAttribute(clickAttr, '1');
    ...
    if (!/No file input found/i.test(result.error)) {
