...
function parseLease(tabIdKey: string, value: unknown): TabLease | undefined {
  ...
  if (instanceId !== null && typeof instanceId !== 'string') return undefined;
...
class TabLeaseManager {
  ...
  private async getInstanceId(): Promise<string | null> {
    ...
    try {
      ...
      this.instanceId =
        typeof stored.extensionInstanceId === 'string' ? stored.extensionInstanceId : null;
    } catch {
  ...
  async resumeHandoffTabs(
    ...
    return await this.withMutation(async () => {
      ...
      for (const [tabId, lease] of this.leases.entries()) {
        ...
        if (typeof lease.groupId === 'number') updated.groupId = lease.groupId;
        this.leases.set(tabId, updated);
  ...
  async releaseSession(sessionId: string): Promise<void> {
    await this.withMutation(async () => {
      let changed = false;
      const changedTabIds: number[] = [];
      for (const [tabId, lease] of Array.from(this.leases.entries())) {
  ...
  async recordTabGroup(tabId: number, groupId: number): Promise<void> {
    await this.withMutation(async () => {
      const lease = this.leases.get(tabId);
      if (!lease || lease.groupId === groupId) return;
      this.leases.set(tabId, { ...lease, groupId });
      await this.saveToStorage();
    });
  }
