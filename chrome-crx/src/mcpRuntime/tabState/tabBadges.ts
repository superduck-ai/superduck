...
class TabBadgeManager {
  ...
  async markDeliverable(tabIds: number[]): Promise<void> {
    ...
    for (const tabId of tabIds) await this.applyBadge(tabId);
  }
  ...
  private async refreshDeliverableBadges(): Promise<void> {
    for (const tabId of this.deliverableTabIds) await this.applyBadge(tabId);
  }

  /**
   * Drop deliverable markers for tabs that no longer exist. Called by the
   * finalize/cleanup path opportunistically; safe to call repeatedly.
   */
  async pruneMissingTabs(): Promise<void> {
    if (this.deliverableTabIds.size === 0) return;
    let changed = false;
    for (const tabId of Array.from(this.deliverableTabIds)) {
      try {
